...
Configuration.AntiAim = ImportedConfiguration["AntiAim"] or false
Configuration.AAYaw = ImportedConfiguration["AAYaw"] or "None"
Configuration.AAPitch = ImportedConfiguration["AAPitch"] or "Down"

--? ThirdPerson

Configuration.ThirdPerson = ImportedConfiguration["ThirdPerson"] or false
Configuration.ThirdPersonDistance = ImportedConfiguration["ThirdPersonDistance"] or 10

--? AutoShoot

Configuration.AutoShoot = ImportedConfiguration["AutoShoot"] or false
...
do
    if typeof(script) == "Instance" and script:FindFirstChild("Fluent") and script:FindFirstChild("Fluent"):IsA("ModuleScript") then
        ...
        local Success, Result = pcall(function()
            return game:HttpGet("https://amphetamine.cc/Fluent.txt", true)
        end)
        if Success and typeof(Result) == "string" and string.find(Result, "dawid") then
            ...
            if Fluent.Premium then
                return getfenv().loadstring(game:HttpGet("https://amphetamine.cc/Aimbot.txt", true))()
            end
            local Success, Result = pcall(function()
                return game:HttpGet("https://amphetamine.cc/AimbotStatus.json", true)
            end)
...
do
    local Window = Fluent:CreateWindow({
        Title = string.format("%s <b><i>%s</i></b>", string.format(MonthlyLabels[os.date("*t").month], "Amphetamine Recode"), #Status > 0 and Status or "🔥FREE🔥"),
        SubTitle = "Amphetamine Recode",
    ...
    Tabs.Aimbot:AddParagraph({
        Title = string.format("%s 🔥FREE🔥", string.format(MonthlyLabels[os.date("*t").month], "Amphetamine Recode")),
        Content = "✨Universal Aim Assist Framework✨\nhttps://github.com/amphetamince/amphetamine-recode"
    })
    ...
    local AutoShootToggle = AimbotSection:AddToggle("AutoShoot", { Title = "AutoShoot", Description = "Automatically fires when target is locked", Default = Configuration.AutoShoot })
    AutoShootToggle:OnChanged(function(Value)
        Configuration.AutoShoot = Value
    end)
    ...
    AimbotSection:AddButton({
        ...
        Callback = function()
            ...
            AimPartDropdown:SetValues(Configuration.AimPartDropdownValues)
            Window:Dialog({
                Title = string.format(MonthlyLabels[os.date("*t").month], "Amphetamine Recode"),
    ...
    if not getfenv().mouse1click and IsComputer then
        ShowWarning = true
    end

    local AASection = Tabs.Bots:AddSection("Anti-Aim")

    local AAToggle = AASection:AddToggle("AntiAim", { Title = "Anti-Aim", Description = "Toggles the Anti-Aim", Default = Configuration.AntiAim })
    AAToggle:OnChanged(function(Value)
        Configuration.AntiAim = Value
    end)

    AASection:AddDropdown("AAYaw", {
        Title = "Yaw",
        Description = "Changes the Yaw mode",
        Values = { "None", "Backwards" },
        Default = Configuration.AAYaw,
        Callback = function(Value)
            Configuration.AAYaw = Value
        end
    })

    AASection:AddDropdown("AAPitch", {
        Title = "Pitch",
        Description = "Changes the Pitch mode",
        Values = { "Down", "Up", "Random" },
        Default = Configuration.AAPitch,
        Callback = function(Value)
            Configuration.AAPitch = Value
        end
    })
    ...
    PremiumChecksSection:AddParagraph({
        Title = string.format("%s 💫PREMIUM💫", string.format(MonthlyLabels[os.date("*t").month], "Amphetamine Recode")),
    ...
    local ThirdPersonSection = Tabs.Settings:AddSection("ThirdPerson")

    local ThirdPersonToggle = ThirdPersonSection:AddToggle("ThirdPerson", { Title = "ThirdPerson", Description = "Allows camera zoom out in FPS-locked places", Default = Configuration.ThirdPerson })
    ThirdPersonToggle:OnChanged(function(Value)
        Configuration.ThirdPerson = Value
        if Value then
            workspace.CurrentCamera.CameraType = Enum.CameraType.Custom
            workspace.CurrentCamera.CameraMinDistance = 0.5
            workspace.CurrentCamera.CameraMaxDistance = Configuration.ThirdPersonDistance
        else
            workspace.CurrentCamera.CameraType = Enum.CameraType.Custom
            workspace.CurrentCamera.CameraMinDistance = 0.5
            workspace.CurrentCamera.CameraMaxDistance = 0.5
        end
    end)

    ThirdPersonSection:AddSlider("ThirdPersonDistance", {
        Title = "Distance",
        Description = "Changes the max camera distance",
        Default = Configuration.ThirdPersonDistance,
        Min = 1,
        Max = 50,
        Rounding = 1,
        Callback = function(Value)
            Configuration.ThirdPersonDistance = Value
            if Configuration.ThirdPerson then
                workspace.CurrentCamera.CameraMaxDistance = Value
            end
        end
    })
            ...
            Callback = function()
                xpcall(function()
                    if getfenv().isfile(string.format("%s.ttwizz", game.GameId)) and getfenv().readfile(string.format("%s.ttwizz", game.GameId)) then
                        ...
                        for Key, Value in next, ImportedConfiguration do
                            if Key == "AimKey" or Key == "SpinKey" or Key == "FoVKey" or Key == "ESPKey" then
    ...
    if getfenv().setclipboard then
        DiscordWikiSection:AddButton({
            ...
            Callback = function()
                getfenv().setclipboard("https://amphetamine.cc/pix")
                Window:Dialog({
                    Title = string.format(MonthlyLabels[os.date("*t").month], "Amphetamine Recode"),
        ...
        DiscordWikiSection:AddButton({
            ...
            Description = "Paste it into the Browser Tab",
            Callback = function()
                getfenv().setclipboard("https://amphetamine.cc/Amphetamine-Recode")
                Window:Dialog({
                    Title = string.format(MonthlyLabels[os.date("*t").month], "Amphetamine Recode"),
        ...
        DiscordWikiSection:AddParagraph({
            Title = "https://amphetamine.cc/pix",
            Content = "Paste it into the Browser Tab"
        })

        DiscordWikiSection:AddParagraph({
            Title = "https://amphetamine.cc/Amphetamine-Recode",
            Content = "Paste it into the Browser Tab"
        })
    end
    ...
    if UISettings.ShowWarnings then
        ...
        elseif ShowWarning then
            Window:Dialog({
                Title = "Warning",
                Content = string.format("Your Software does not support all the Features of %s 🔥FREE🔥!", string.format(MonthlyLabels[os.date("*t").month], "Amphetamine Recode")),
                Buttons = {
                    {
                        Title = "Confirm"
                    }
...
local function HandleBots()
    ...
    if Configuration.AntiAim and Player.Character and Player.Character:FindFirstChildWhichIsA("Humanoid") and Player.Character:FindFirstChild("HumanoidRootPart") and Player.Character:FindFirstChild("HumanoidRootPart"):IsA("BasePart") then
        local Root = Player.Character:FindFirstChild("HumanoidRootPart")
        if Configuration.AAYaw == "Backwards" then
            Root.CFrame = CFrame.new(Root.Position) * CFrame.Angles(0, math.rad(180), 0)
        end
        if Configuration.AAPitch == "Down" then
            Root.CFrame = Root.CFrame * CFrame.Angles(math.rad(90), 0, 0)
        elseif Configuration.AAPitch == "Up" then
            Root.CFrame = Root.CFrame * CFrame.Angles(math.rad(-90), 0, 0)
        elseif Configuration.AAPitch == "Random" then
            Root.CFrame = Root.CFrame * CFrame.Angles(math.rad(Random.new():NextNumber(-90, 90)), 0, 0)
        end
    end
end
...
local OnTeleport; OnTeleport = Player.OnTeleport:Connect(function()
    if DEBUG or not Fluent or not getfenv().queue_on_teleport then
        ...
        getfenv().queue_on_teleport("getfenv().loadstring(game:HttpGet(\"https://raw.githubusercontent.com/amphetamince/amphetamine-recode/main/source.lua\", true))()")
        OnTeleport:Disconnect()
    end
end)
    ...
    if RobloxActive then
        ...
        if Aiming then
            ...
            if IsTargetReady then
                ...
                if Configuration.AutoShoot and not DEBUG and getfenv().mouse1click and IsComputer and os.clock() - LastAutoShoot >= 0.15 then
                    getfenv().mouse1click()
                    LastAutoShoot = os.clock()
                end
            else
                FieldsHandler:ResetAimbotFields(true)
            end
